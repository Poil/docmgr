...
/************************************************************************
*************************************************************************/
function employeeCheckbox($formInfo) {

	$conn = $formInfo["conn"];

	$filter = "(locationId=".CURUSER_LOCATION.")(employeeType=agent)";
	$option["search_filter"] = $filter;

	$list = returnAccountList($option);
	
	$content .= "<div class=\"formHeader\">";
	$content .= "Agent";
	$content .= "</div>";

	$content .= "<table cellpadding=0 cellspacing=0><tr><td valign=top><table>";

	$num = count($list["id"]);
	$check = intValue($num/2);

	for ($row=0;$row<$num;$row++) {
	
		if ($row == $check) $content .= "</table></td><td valign=top><table>";

		if (is_array($formInfo["cur_value"]) && in_array($list["id"][$row],$formInfo["cur_value"])) $checked = " CHECKED ";
		else $checked = null;
		...
		$content .= "<input 	type=checkbox 
					...
					";
		$content .= "</td><td>";
		$content .= $list["first_name"][$row]." ".substr($list["last_name"][$row],0,1).".";
		$content .= "</td></tr>";
...
/************************************************************************
*************************************************************************/
function employeeDropdown($formInfo) {
	...
	for ($row=0;$row<count($list["id"]);$row++) {
		...
		$content .= "<option ".$selected." value=\"".$list["id"][$row]."\">
				".$list["last_name"][$row].", ".$list["first_name"][$row];
...
function dateRange($formInfo) {
	...
	$option1["name"] = "From";
	$option1["form_name"] = $formInfo["form_name"]."Begin";

	$option2["name"] = "To";
	$option2["form_name"] = $formInfo["form_name"]."End";
...
/************************************************************************
*************************************************************************/
function dateForm1($title,$name,$value) {
	...
	if (defined("DATE_FORMAT")) $dateForm = DATE_FORMAT;
	else $dateForm = "mm/dd/yyyy";

	$tempForm = $dateForm;

	$tempForm = str_replace("mm","Month",$tempForm);	
	$tempForm = str_replace("dd","Day",$tempForm);	
	$tempForm = str_replace("yyyy","Year",$tempForm);	

	$dateArray = explode("/",$tempForm);
	
	for ($row=0;$row<count($dateArray);$row++) {
		...
		$keyDown = "fieldJump('".$name.$cur."','".$name.$prev."','".$name.$next."');";
		$onSelect = "saveCaret('".$name.$cur."');";
		$onFocus = "saveCaret('".$name.$cur."');";

		if ($cur=="Month") $val = &$month;
		elseif ($cur=="Day") $val = &$day;
		elseif ($cur=="Year") $val = &$year;

		$val = str_replace("KEYDOWN",$keyDown,$val);
		$val = str_replace("ONSELECT",$onSelect,$val);
		$val = str_replace("ONFOCUS",$onFocus,$val);

	}

	$dateForm = str_replace("/","-",$dateForm);
	$dateForm = str_replace("mm",$month,$dateForm);	
	$dateForm = str_replace("dd",$day,$dateForm);	
	$dateForm = str_replace("yyyy",$year,$dateForm);	
...
*************************************************************************/
function dateFormSelect($header,$formName,$formValue,$readonly="READONLY") {
	...
	$str = "<div class=\"formHeader\">
		...
		<input type=text size=10 ".$readonly." name=\"".$formName."\" id=\"".$formName."\" value=\"".$formValue."\">
		<input type=button name=\"".$btnName."\" id=\"".$btnName."\" value=\"...\">
		";

	//call our js function and setup loaders
	$str .= jsCalLoad();
	$str .= jsCalSetup($formName,$btnName);
...
function functionMenu($tableName,$nameList,$functionList) {
    ...
    if (BROWSER=="mozilla") $leave = "onBlur=\"document.getElementById('".$tableName."').style.visibility='hidden';\"";
