...
function removeRevision($conn,$objectId,$fileId) {
	...
	if ($fileId=="earliest") {
		...
		//config check
		if (!defined("FILE_REVISION_LIMIT") || FILE_REVISION_LIMIT=="0") return false;

		$sql = "SELECT id FROM dm_file_history WHERE object_id='$objectId' ORDER BY id";	
		...
		if ($info["count"] > FILE_REVISION_LIMIT) {
			  ...
			  $sql = "DELETE FROM dm_file_history WHERE id='$fileId'";
			  if (db_query($conn,$sql)) {
				$file = DATA_DIR."/".returnObjPath($conn,$objectId)."/".$fileId.".docmgr";
				@unlink($file);
                          }
		...
		//config check
		if (!defined("FILE_REVISION_REMOVE") || FILE_REVISION_REMOVE=="no") return false;
		
		//get the latest version of this file
		$sql = "SELECT version FROM dm_file_history WHERE id='".$fileId."'";
		...
		if (db_query($conn,$sql)) {
			...
			//promote our file, then reindex and rethumb
	  	  	fileCommon::promote($conn,$objectId,$info["id"]);
...
function promote($conn,$objectId,$fileId) {
	...
	for ($row=0;$row<$num;$row++) {
		...
		$version = $num - $row;
		$sql .= "UPDATE dm_file_history SET version='$version' WHERE id='$temp[$row]';";

	}

	//get the new name to set our object to
	$sqltmp = "SELECT name FROM dm_file_history WHERE id='$fileId'";
	$arr = single_result($conn,$sqltmp);

	//update the database to use the highest one if there is a name field in the database
	$sql .= "UPDATE dm_object SET name='".sanitizeString($arr["name"])."',version='".max($version_array)."' WHERE id='$objectId';";
