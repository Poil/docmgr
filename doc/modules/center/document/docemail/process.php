...
if ($pageAction=="send") {
	...
	$sql = "SELECT id,name,version,
			(SELECT id FROM dm_document WHERE dm_document.object_id = dm_object.id AND dm_document.version=dm_object.version) AS doc_id
			FROM dm_object WHERE id='$objectId'";
	$info = single_result($conn,$sql);

	//create our file path
	$filePath = DOC_DIR."/".returnObjPath($conn,$objectId)."/".$info["doc_id"].".docmgr";
	...
	if ($emailTo && $emailFrom) {
		...
		if ($_POST["anonRecip"]) {
			...
			$subject = _EMAIL_DOC." \"".$info["name"]."\"";
			$emailMessage = $emailComments."<hr>";
			$emailMessage .= file_get_contents($filePath);

			if (send_email($emailTo,$emailFrom,$subject,$emailMessage,null)) {
