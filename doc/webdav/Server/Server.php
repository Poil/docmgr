...
// +----------------------------------------------------------------------+
//
// $Id: Server.php,v 1.52 2006/07/08 16:01:10 hholzgra Exp $
//
...
class HTTP_WebDAV_Server 
{
    ...
    */
    ...
    function http_LOCK() 
    {
        ...
        webdavfunc::checkOutput("http status is ".$http_stat."\n");

        $this->http_status($http_stat);
        
        if ($http_stat[0] == 2) { // 2xx states are ok 
        
            webdavfunc::checkOutput("here\n");
            $timeout = "Infinite";
            /*
            if($options["timeout"]) {
            ...
            */
            webdavfunc::checkOutput("there\n");

            $str = null;
            header('Content-Type: text/xml; charset="utf-8"');
            header("Lock-Token: <".$options[locktoken].">");
            $str .= "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
            $str .= "<D:prop xmlns:D=\"DAV:\">\n";
            $str .= " <D:lockdiscovery>\n";
            $str .= "  <D:activelock>\n";
            $str .= "   <D:lockscope><D:$options[scope]/></D:lockscope>\n";
            $str .= "   <D:locktype><D:$options[type]/></D:locktype>\n";
            $str .= "   <D:depth>$options[depth]</D:depth>\n";
            $str .= "   <D:owner>$options[owner]</D:owner>\n";
            $str .= "   <D:timeout>$timeout</D:timeout>\n";
            $str .= "   <D:locktoken><D:href>$options[locktoken]</D:href></D:locktoken>\n";
            $str .= "  </D:activelock>\n";
            $str .= " </D:lockdiscovery>\n";
            $str .= "</D:prop>\n\n";

            webdavfunc::checkOutput($str);
            echo $str;
