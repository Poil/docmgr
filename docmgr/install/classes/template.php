...
class TEMPLATE {
  ...
  ********************************************************************/  
  function buildForm($sectionArr = null)
  {
    ...
    foreach ($this->template AS $sectionName=>$section)
    {
      ...
      if (isset($section["hidden"])) continue;
  ...
  ********************************************************************/  
  function mergePost($filterArr = null)
  {
    ...
    foreach ($filterArr AS $sectionName)
    {
      ...
      foreach ($keys AS $entry)
      {
        ...
        if (!isset($_POST[$entry]) && !$section[$entry][3])
        {
    ...
    if (file_exists($this->configFile))
    {
      ...
      foreach ($keys AS $key)
      {
        ...
        foreach ($forms AS $form)
        {
          ...
          //this happens if it's a new parameter for the newer version, not going
          //to be in the older one
  ...
  function writeFile()
  {
    ...
    if (isset($this->template["Suffix"])) $output .= $this->template["Suffix"];
